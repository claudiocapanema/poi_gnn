...
class PoiCategorizationTransferLearningDomain:
    ...
    def adjacency_preprocessing(self,
        ...
        for i in range(len(ids)):
            ...
            user_matrices, user_matrices_week, user_matrices_weekend, user_category, idxs, number_of_matrices = self._resize_adjacency_and_category_matrices(user_matrices, user_matrices_week, user_matrices_weekend, user_category, max_size_matrices, dataset_name)
    ...
    def k_fold_with_replication_train_and_evaluate_model(self,
        ...
        y_predicted = self.train_and_evaluate_model(data,
                                                                        max_size_matrices,
                                                                        epochs,
                                                                        seed,
                                                                        country,
    ...
    def train_and_evaluate_model(self,
        ...
        adjacency_weekend_df, temporal_weekend_df, location_time_df, location_location_df = data

        data = adjacency_df, adjacency_week_df, adjacency_weekend_df, temporal_df, temporal_week_df, temporal_weekend_df, distance_df, duration_df, location_time_df, location_location_df
        ...
        print("Tamanho das matrizes de treino: ", users_categories.shape, adjacency_df.shape, temporal_df.shape, distance_df.shape, duration_df.shape, adjacency_week_df.shape, temporal_week_df.shape,\
        adjacency_weekend_df.shape, temporal_weekend_df.shape, location_time_df.shape, location_location_df.shape)
        ...
        user_index = max_user
        self.heatmap_matrices(str(0), [adjacency_df[0], adjacency_week_df[0], adjacency_weekend_df[0],
                               temporal_df[0], temporal_week_df[0], temporal_weekend_df[0], location_time_df[0], location_location_df[0]],
                              ["Adjacency", "Adjacency (weekday)", "Adjacency (weekend)", "Temporal", "Temporal (weekday)", "Temporal (weekend)", "Location_time", "Location_location"],
                              output_dir)

        y_predict_location = model.predict(data,
                                           batch_size=batch)
